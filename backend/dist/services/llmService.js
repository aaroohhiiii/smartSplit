 ...
 */
export const parseBillWithGroq = async (fileBuffer, filePath) => {
...
5. All amounts should be numeric values`;
    try {
        ...
        if (!response.ok) {
            ...
            const errorMessage = typeof error === 'object' && error !== null && 'error' in error && typeof error.error?.message === 'string' ? error.error.message : "Unknown error";
 ...
 */
export const validateBillParsing = (result) => {
    const errors = [];
